// vendedor/pedidos.js – Vendedor (con descuento de inventario al entregar)
window.pedidos = {
...
window.pedidos.eliminarSeleccionados = function() {
  ...
  document.getElementById('btn-confirmar-eliminar').onclick = async () => {
    ...
    const { error } = await window.vendedorSupabase.from('pedidos').delete().in('id', ids);
    if (error) {
...
window.pedidos.cambiarEstado = async function(id, nuevoEstado) {
  ...
  const { error } = await window.vendedorSupabase
    .from('pedidos')
...
// ─── Función de descuento de inventario (idéntica a la del admin) ───
async function descontarStockDePedido(pedidoId) {
  console.log(`[Inventario] Iniciando descuento para pedido ${pedidoId}`);
  const { data: pedido, error: errPedido } = await window.vendedorSupabase
    .from('pedidos')
    .select('items, estado')
    .eq('id', pedidoId)
    .single();
  ...
  for (const item of items) {
    ...
    for (const sub of subitems) {
      ...
      if (!productoId) {
        ...
        const { data: productos } = await window.vendedorSupabase
          .from('productos')
          .select('id, stock')
        ...
        const { data: producto } = await window.vendedorSupabase
          .from('productos')
          .select('stock')
          .eq('id', productoId)
          .single();
      ...
      const { error: errUpdate } = await window.vendedorSupabase
        .from('productos')
      ...
      const { error: errMov } = await window.vendedorSupabase
